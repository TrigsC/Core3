...
SortedVector<String> PlayerManagerImplementation::getTeachableSkills(CreatureObject* teacher, CreatureObject* student) {
	...
	for (int i = 0; i < skillList->size(); ++i) {
		...
		if (!(skillName.contains("novice") || skillName.contains("force_sensitive") || skillName.contains("force_rank") || skillName.contains("force_title") || skillName.contains("faction_") || skillName.contains("admin_")) && skillManager->canLearnSkill(skillName, student, false))
